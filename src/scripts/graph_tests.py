...
def generate_subgraphs(graph, size):
    """
    Generate unique subgraphs of a given size from a graph and count their occurrences.
    It uses combination (num_nodes, size) to generate all possible subgraphs of the given size.
    ...
    Returns:
        dict: Dictionary where keys are subgraphs (as frozen sets of edges) and values are their occurrences.
    """
    subgraph_dict = {}

    for nodes in itertools.combinations(graph.nodes(), size):
        # Generate subgraph from selected nodes
        subgraph = graph.subgraph(nodes)
        
        # Ensure the subgraph is connected
        if nx.is_weakly_connected(subgraph) or nx.is_strongly_connected(subgraph):
            # Check if this subgraph is isomorphic to any already stored in subgraph_dict
            found_isomorphic = False
            for key, data in subgraph_dict.items():
                # If we find an isomorphic subgraph, increment the occurrence count
                if nx.vf2pp_is_isomorphic(subgraph, data['subgraph']):
                    subgraph_dict[key]['occurrences'] += 1
                    found_isomorphic = True
                    break
            
            # If no isomorphic subgraph was found, add the new subgraph to the dictionary
            if not found_isomorphic:
                subgraph_key = frozenset(subgraph.edges())
                subgraph_dict[subgraph_key] = {'subgraph': subgraph, 'occurrences': 1}
...
def subgraph_count(graph, motifs, anchored=False):
    ...
    if anchored:
        ...
        # Generate the subgraphs and count their occurrences
        all_subgraphs = generate_subgraphs(graph, max_size)

        # Iterate through motifs and count occurrences in the unique subgraphs
        for i, motif in enumerate(motifs):
            for _, data in all_subgraphs.items():
                # Check if the subgraph and motif are isomorphic (same structure)
                if len(data['subgraph'].edges()) == len(motif.edges()):
                    if nx.vf2pp_is_isomorphic(data['subgraph'], motif):
                        # Add the occurrences of the matching subgraph to the motif count
                        motif_counts[i] += data['occurrences']
